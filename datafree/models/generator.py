...
class RandomGenerator(nn.Module):
    ...
    def forward(self, targets=None):
        batch_size = targets.shape[0] if targets is not None else 1
        random_img = torch.randn(batch_size, self.nc, self.img_size, self.img_size).to(next(self.parameters()).device)
        return random_img
    
    def reinit(self):
        return RandomGenerator(self.params[0], self.params[1], self.params[2], self.params[3], self.params[4],
                               self.params[5], self.params[6], self.params[7]).cuda()