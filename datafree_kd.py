...
# Data Free
parser.add_argument('--method', default='nldf', choices=['nldf', 'nayer', 'random'])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.method == 'random':
        ...
        if args.dataset == 'cifar10' or args.dataset == 'cifar100':
            generator = datafree.models.generator.RandomGenerator(ngf=64, img_size=32, nc=3, nl=num_classes,
                                                                  label_emb=label_emb, le_emb_size=args.le_emb_size,
                                                                  sbz=args.synthesis_batch_size)
            generator = prepare_model(generator)
            synthesizer = datafree.synthesis.NAYER(teacher, student, generator,
...
def train(synthesizer, model, criterion, optimizer, args):
    ...
    save_dir = os.path.join("generated_images", args.method, current_date)
    ...
    for i in range(args.kd_steps):
        ...
        if args.current_epoch % 20 == 0 and i % 50 == 0:
            vutils.save_image(images, os.path.join(save_dir, f"image_epoch{args.current_epoch}_iter{i}.png"))
